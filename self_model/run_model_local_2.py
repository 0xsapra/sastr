...
# MODEL_PATH = os.path.expanduser("./qwen-7b")
...
def parse_tool_call(text):
    ...
    # 2. Try parsing raw text if it looks like JSON
...
async def chat_completions(request: Request):
    ...
    try:
        # Pass tools to tokenizer to get correct prompt format
        ...
        # Fallback logic in case of template errors
        print(f"Template Warning: {e}. Retrying without tools.")
        try:
            prompt = tokenizer.apply_chat_template(
                messages,
                tokenize=False, 
            ...
            raise HTTPException(status_code=400, detail=f"Template Error: {str(e2)}")
    ...
    if stream:
        # NOTE: Streaming tool calls is complex and requires a state machine parser.
        # For this simple server, we only support streaming TEXT. 
        # If the model generates a tool call in streaming mode, LangChain might fail to parse it.
        ...
        # --- PARSE TOOL CALLS ---
        # We check if the output text is actually a tool call JSON
        tool_call_data = parse_tool_call(full_text)
        
        message_content = full_text
        tool_calls = None
        finish_reason = "stop"

        # If we found a valid tool call structure
        if tool_call_data and "name" in tool_call_data and "arguments" in tool_call_data:
             print(f"--- Detected Tool Call: {tool_call_data['name']} ---")
             
             # OpenAI expects content to be null when tool_calls are present
             message_content = None 
             finish_reason = "tool_calls"
             
             # Ensure arguments is a dictionary before dumping to string
             # This prevents double-stringification if the model output arguments as a string
             args_payload = tool_call_data["arguments"]
             if isinstance(args_payload, str):
                 try:
                     args_payload = json.loads(args_payload)
                 except:
                     # Fallback: if it's a string we can't parse, keep it (might fail downstream)
                     pass
             
             # Construct the OpenAI-compatible tool_call object
             tool_calls = [{
                 "id": f"call_{uuid.uuid4().hex[:8]}",
                 "type": "function",
                 "function": {
                     "name": tool_call_data["name"],
                     # Arguments must be a JSON STRING representing a dictionary
        ...
        return {
            ...
            "choices": [{
                ...
                "message": {
                    ...
                    "tool_calls": tool_calls, # This triggers LangChain!
                },
                "finish_reason": finish_reason
            }],
            "usage": {
